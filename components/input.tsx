...
const Input = ({ isEditing, itemToEdit }: InputProps) => {
  ...
  const createTodo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to ${isEditing ? "Edit" : "Create"} Todo: ${response.statusText
          }`
        );
      }
  ...
  return (
    ...
    >
      ...
      />
      <button
        className={`rounded-r-sm w-24 text-white bg-slate-500/75 font-bold hover:bg-slate-500 ${todoTitle && "bg-slate-500"
          }`}
        type="submit"
        disabled={isLoading}
      >
